...
app.get("/products", (req, res) => {
  ...
  if (!maxPrice && !limit) res.send(products);
  else if (maxPrice && limit)
    res.send(
      products.filter(
        (product) => product.id < limit && product.price < maxPrice
      )
    );
  else if (maxPrice)
    res.send(products.filter((product) => product.price < maxPrice));
  else if (limit) res.send(products.filter((product) => product.id < limit));
});
